...
impl WebGpuEngine {
    ...
    /// GPU-accelerated multi-block FSE decode.
    ///
    /// Decodes multiple independently-encoded FSE blocks in a single GPU
    /// dispatch. Each block has N interleaved streams, and each stream
    /// gets one work-item within the block's workgroup.
    ///
    /// All blocks must share the same frequency table (from the first block's
    /// header). Returns concatenated decoded output from all blocks.
    pub fn fse_decode_blocks(&self, encoded_blocks: &[(&[u8], usize)]) -> PzResult<Vec<u8>> {
        ...
        // Pad bitstream data to u32 alignment + 4 bytes for safe reads.
        while !all_bitstream_data.len().is_multiple_of(4) {
            all_bitstream_data.push(0);
        }
        ...
        // Combine block_meta and stream_meta into a single buffer to stay
        // within WebGPU's max_storage_buffers_per_shader_stage limit (4).
        let bm_count = block_metas.len();
        let mut combined_meta = block_metas;
        combined_meta.extend_from_slice(&all_stream_meta);
        ...
        // Bind group (4 storage + 1 uniform = within WebGPU limits).
        let bg_layout = self.pipeline_fse_decode_blocks().get_bind_group_layout(0);
        let bg = self.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("fse_db_bg"),
            layout: &bg_layout,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
        ...
        self.dispatch(
            self.pipeline_fse_decode_blocks(),
            &bg,
            num_blocks as u32,
            "fse_decode_blocks",
        )?;
        ...
        // Read output.
        let raw_output = self.read_buffer(&output_buf, (output_u32_count * 4) as u64);
        let result = raw_output[..total_output].to_vec();
