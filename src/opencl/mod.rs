...
impl OpenClEngine {
    ...
    fn create(prefer_gpu: bool, profiling: bool) -> PzResult<Self> {
        ...
        // Helper: compile an OpenCL program, printing elapsed time when profiling.
        let compile = |label: &str, source: &str, flags: &str| -> Result<Program, PzError> {
            let t0 = std::time::Instant::now();
            let prog = Program::create_and_build_from_source(&context, source, flags)
                .map_err(|_| PzError::Unsupported)?;
            if profiling {
                let ms = t0.elapsed().as_secs_f64() * 1000.0;
                eprintln!("[pz-gpu] compile {label}: {ms:.3} ms");
            }
            Ok(prog)
        };

        // Compile all kernel programs
        let program_per_pos = compile("lz77.cl", LZ77_KERNEL_SOURCE, "-Werror")?;
        let program_batch = compile("lz77_batch.cl", LZ77_BATCH_KERNEL_SOURCE, "-Werror")?;
        ...
        let program_topk = compile("lz77_topk.cl", LZ77_TOPK_KERNEL_SOURCE, "-Werror")?;

        let kernel_topk =
            Kernel::create(&program_topk, "EncodeTopK").map_err(|_| PzError::Unsupported)?;

        let program_hash = compile("lz77_hash.cl", LZ77_HASH_KERNEL_SOURCE, "-Werror")?;
        ...
        let program_bwt_rank = compile("bwt_rank.cl", BWT_RANK_KERNEL_SOURCE, &rank_flags)?;
        ...
        let program_bwt_radix = compile("bwt_radix.cl", BWT_RADIX_KERNEL_SOURCE, &rank_flags)?;
        ...
        let program_huffman =
            compile("huffman_encode.cl", HUFFMAN_ENCODE_KERNEL_SOURCE, "-Werror")?;
        ...
        let program_fse_decode = compile("fse_decode.cl", FSE_DECODE_KERNEL_SOURCE, "-Werror")?;
