...
| **Pz2d** | sequences (own wire) | 8-lane Huffman | Pz2 dict tier (`-p pz2d`): 32 MiB segments, first 16 MiB doubles as shared dict (frozen-finder encode, 2-wave prefix-primed decode). **Blob 30.5%** — best LZ-family ratio in pz, 0.9pp under pzstd-3 — at 42.5 ms decode (v1: memory-traffic-bound by per-block dict priming; arena decode projected ~17-20 ms) and 23.7 s encode (concurrent dict-walk cache inflation 2.9x). Opt-in max-ratio tier; ledger in clean-slate-codec.md §11b. |
