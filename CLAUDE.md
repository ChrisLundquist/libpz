...
| **Pz2d** | sequences (own wire) | 8-lane Huffman | Pz2 dict tier (`-p pz2d`): 32 MiB segments, first 16 MiB doubles as shared dict (frozen-finder encode, 2-wave arena decode). **Blob 30.5%** — best LZ-family ratio in pz, 0.9pp under pzstd-3 — at ~39 ms decode (v2 arena decode, −13% vs v1; wall is DRAM-latency-bound on random dict reads, NOT traffic-bound — do not chase further copy elimination or the two-region splice, see §11c; absolute walls drift ±10% with machine state, compare within one hyperfine run) and 14.1 s encode (DICT_CHAIN_CAP=16 dict-walk budget, −44% wall for +0.06pp, §11d). Opt-in max-ratio tier; ledger in clean-slate-codec.md §11b-§11d. |
...
- **Segment-global entropy tables and order-1 seq-code modeling are measured dead for pz2** (entropy probe, 2026-06-10): global tables are +0.89pp WORSE on the blob — 2 MiB per-block histograms are already statistically saturated, and heterogeneous segments poison shared tables (same lesson as the global head dict); order-1 segment-global is −0.055pp, under the gate. The live candidate the probe surfaced instead: **per-block tANS on the three sequence-code lanes** (−0.20 to −0.25pp, gate-passing; the win is the Huffman 1-bit floor on rep0/zero-run-dominated code lanes, literals are already near-optimal) — but any wire change is gated on a decode-speed-neutral prototype of the fused splice with FSE states. See `docs/design-docs/clean-slate-codec.md` §12 + `examples/pz2_entropy_probe.rs`.
