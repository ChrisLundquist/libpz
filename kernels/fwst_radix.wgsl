...
@group(0) @binding(3) var<uniform> fk_params: vec4<u32>; // x=n, y=padded_n, z=pass, w=unused
...
fn fwst_compute_keys(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Key = input[(sa_i + pass) % n], reading from packed u32 array.
    let byte_idx = (sa_i + pass) % n;
