...
fn bench_compress(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("compress_lzf");
    cap(&mut group);
    group.throughput(Throughput::Bytes(data.len() as u64));

    group.bench_with_input(BenchmarkId::new("pz", "Lzf"), &data, |b, data| {
        b.iter(|| pipeline::compress(data, Pipeline::Lzf).unwrap());
    });
    group.finish();
}
...
fn bench_decompress(c: &mut Criterion) {
    let data = get_test_data();
    let mut group = c.benchmark_group("decompress_lzf");
    cap(&mut group);
    group.throughput(Throughput::Bytes(data.len() as u64));

    group.bench_function(BenchmarkId::new("pz", "Lzf"), |b| {
        let compressed = pipeline::compress(&data, Pipeline::Lzf).unwrap();
...
fn bench_compress_parallel(c: &mut Criterion) {
    ...
    let data = get_test_data();
    let mut group = c.benchmark_group("compress_parallel_lzf");
    cap(&mut group);
    group.throughput(Throughput::Bytes(data.len() as u64));

    group.bench_with_input(BenchmarkId::new("pz_mt", "Lzf"), &data, |b, data| {
        let opts = CompressOptions {
            threads: 0,
            ..Default::default()
        };
        b.iter(|| pipeline::compress_with_options(data, Pipeline::Lzf, &opts).unwrap());
    });
...
fn bench_compress_large(c: &mut Criterion) {
    let mut group = c.benchmark_group("compress_large_lzf");
    cap(&mut group);

    for &size in &[4_194_304usize, 16_777_216] {
        let data = get_test_data_sized(size);
        group.throughput(Throughput::Bytes(size as u64));
        group.bench_with_input(BenchmarkId::new("Lzf", size), &data, |b, data| {
            b.iter(|| pipeline::compress(data, Pipeline::Lzf).unwrap());
        });
    }
...
fn bench_decompress_large(c: &mut Criterion) {
    let mut group = c.benchmark_group("decompress_large_lzf");
    cap(&mut group);

    for &size in &[4_194_304usize, 16_777_216] {
        let data = get_test_data_sized(size);
        group.throughput(Throughput::Bytes(size as u64));
        group.bench_function(BenchmarkId::new("Lzf", size), |b| {
            let compressed = pipeline::compress(&data, Pipeline::Lzf).unwrap();
            b.iter(|| pipeline::decompress(&compressed).unwrap());
        });
    }
...
fn bench_compress_webgpu(c: &mut Criterion) {
    ...
    let data = get_test_data();
    let mut group = c.benchmark_group("compress_webgpu_lzf");
    cap(&mut group);
    group.throughput(Throughput::Bytes(data.len() as u64));
    ...
    group.bench_with_input(
        BenchmarkId::new("pz_webgpu", "Lzf"),
        &data,
        move |b, data| {
            b.iter(|| pipeline::compress_with_options(data, Pipeline::Lzf, &options).unwrap());
        },
    );
...
fn bench_compress_webgpu_large(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("compress_webgpu_large_lzf");
    cap(&mut group);
    ...
    for &size in &[4_194_304usize, 16_777_216] {
        ...
        group.bench_with_input(
            BenchmarkId::new("Lzf_webgpu", size),
            &data,
            move |b, data| {
                b.iter(|| pipeline::compress_with_options(data, Pipeline::Lzf, &options).unwrap());
            },
